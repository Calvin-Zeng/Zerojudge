...
//主程式
int main() {
	string intput_1, intput_2,output;
	int bit1,bit2,bit_ori;
	while (cin >> intput_1>> intput_2>> bit1)
	{
		intput_2_ori = intput_2;
		output = exc(intput_1, intput_2);
		bit_ori = output.size();
		cout << output <<".";
		if (comparison_of_bit_size(intput_1, intput_2) == 1 | comparison_of_bit_size(intput_1, intput_2) == 0) {
			supplement(bit1, intput_1.size(), &intput_1);
			output = exc(intput_1, intput_2);
			output = intput_2.assign(output, bit_ori, output.size());
		}
		else if (comparison_of_bit_size(intput_1, intput_2) == 2) {
			if(intput_2.size() - intput_1.size() > 0)
				for (int i = 0; i < intput_2.size() - intput_1.size(); i++)
				{
					output+="0";
				}
			supplement(bit1, intput_1.size(), &intput_1);
			output += exc(intput_1, intput_2);
			output = output.assign(output, bit_ori, bit1);
		}
		cout << output << endl;
	}
	return 0;
